...
export default async function CategoryPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-[240px_1fr]">
      ...
      <section>
        <nav className="mb-2 text-xs text-white/50">
          <Link href="/shop" className="hover:text-white">Shop</Link>
          {cat.parent && <> / <Link href={`/shop/c/${cat.parent.slug}`} className="hover:text-white">{cat.parent.name}</Link></>}
          {" / "}<span className="text-white">{cat.name}</span>
        </nav>
        <h1 className="mb-1 text-3xl font-black tracking-tight">{cat.name}</h1>
        <p className="mb-6 text-white/60">{products.length} {products.length === 1 ? "product" : "products"}</p>
        {products.length === 0 ? (
          <div className="card text-center text-white/60">No products in this category yet.</div>
        ) : (
