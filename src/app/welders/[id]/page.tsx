    ...
    <div className="grid gap-6 md:grid-cols-[2fr_1fr]">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-3 text-sm text-white/60">
              ...
              {avg && <span className="text-amber-400">★ {avg} ({welder.receivedReviews.length})</span>}
            </div>
          </div>
        </div>
        ...
        {gallery.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {gallery.map((url) => (
                <a key={url} href={url} target="_blank" rel="noopener noreferrer" className="aspect-square overflow-hidden rounded-xl border border-white/10 bg-white/5">
          ...
          <ul className="space-y-3">
            {welder.receivedReviews.map((r) => (
              <li key={r.id} className="rounded-xl border border-white/10 bg-white/5 p-3">
                <div className="text-amber-400">{"★".repeat(r.rating)}<span className="text-white/30">{"★".repeat(5 - r.rating)}</span></div>
      ...
      <aside className="card h-fit space-y-4">
        <div>
          ...
          <div className="flex flex-wrap gap-2 text-xs">{splitList(p?.skills).map((s) => <span key={s} className="rounded-full bg-white/5 px-2 py-1">{s}</span>)}</div>
        </div>
        ...
        {languages.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-2 text-xs">{languages.map((l) => <span key={l} className="rounded-full bg-white/5 px-2 py-1">{l}</span>)}</div>
          </div>
        )}
        ...
        {serviceCountries.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-2 text-xs">{serviceCountries.map((c) => <span key={c} className="rounded-full bg-white/5 px-2 py-1">{c}</span>)}</div>
          </div>
        )}
        ...
        <div>
          ...
          {p?.hourlyRate && <p className="mt-1 text-sm text-white/70">Rate: ${p.hourlyRate}/hr</p>}
        </div>
