    ...
    <div>
      <div className="mb-6 flex items-end justify-between">
        <div>
          ...
          <p className="text-white/60">Browse opportunities from project owners across the network.</p>
        </div>
      ...
      {jobs.length === 0 ? <p className="text-white/60">No open jobs match.</p> : (
          ...
          {jobs.map((j) => (
            <Link key={j.id} href={`/jobs/${j.id}`} className="card flex items-start justify-between gap-4 transition hover:border-amber/40">
              <div>
                ...
                <p className="mt-1 text-sm text-white/70 line-clamp-2">{j.description}</p>
              ...
              {j.budget && <div className="text-right"><div className="text-amber font-bold">${j.budget}</div><div className="text-xs text-white/50">budget</div></div>}
            </Link>
          ))}
        </div>
      )}
