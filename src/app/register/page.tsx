...
export default function RegisterPage() {
  return <Suspense fallback={<div className="mx-auto max-w-md py-12 text-white/60">Loading…</div>}><RegisterInner /></Suspense>;
}
...
function RegisterInner() {
  ...
  return (
    <div className="mx-auto max-w-md py-12">
      ...
      <p className="mb-6 text-white/60">Choose how you want to use HubWeld.</p>
      <form onSubmit={submit} className="card space-y-4">
        <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
          {(["CUSTOMER", "WELDER", "SUPPLIER"] as const).map((r) => (
            <button key={r} type="button" onClick={() => set("role", r)}
              className={`rounded-xl border p-3 text-left text-sm transition ${form.role === r ? "border-amber/60 bg-amber/10" : "border-white/10 bg-white/5 hover:bg-white/10"}`}>
              <div className="font-bold">{r === "CUSTOMER" ? "I need welding work" : r === "WELDER" ? "I provide welding services" : "I sell welding supplies"}</div>
              <div className="text-white/60">{r === "CUSTOMER" ? "Post jobs, receive bids" : r === "WELDER" ? "Get matched to jobs" : "List products on the marketplace"}</div>
            </button>
          ))}
        </div>
        ...
        <p className="text-center text-sm text-white/60">Already have an account? <Link href="/login" className="text-amber hover:underline">Sign in</Link></p>
      </form>
    </div>
  );
}
