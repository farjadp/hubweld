    ...
    <div>
      ...
      <section className="py-16 border-b border-white/8">
        <div className="flex items-center gap-2 mb-5">
          ...
          <span className="text-xs font-black uppercase tracking-widest text-red-400">The HubWeld Story</span>
        </div>
        <h1 className="text-5xl font-black tracking-tight text-white md:text-7xl max-w-4xl leading-[1.05]">
          We are building the<br />
          <span className="bg-gradient-to-r from-red-400 to-red-300 bg-clip-text text-transparent">future of welding.</span>
        </h1>
        ...
        <div className="mt-10 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-white/8 bg-white/8 sm:grid-cols-4 max-w-2xl">
          {stats.map(({ value, label }) => (
            <div key={label} className="flex flex-col items-center gap-1 bg-[#0a0c0e] px-6 py-5 text-center">
      ...
      {/* Mission */}
      <section className="py-16 border-b border-white/8 grid gap-10 lg:grid-cols-2 lg:items-center">
        <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            <div key={label} className="rounded-xl border border-white/8 bg-[#111315] p-5">
              <div className="mb-3 grid h-9 w-9 place-items-center rounded-lg bg-red-600/10 border border-red-600/20 text-red-400">
                <Icon size={16} />
              </div>
      ...
      <section className="py-16 border-b border-white/8">
        ...
        <h2 className="mt-3 text-3xl font-black tracking-tight text-white mb-10">What we stand for</h2>
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {values.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="group rounded-xl border border-white/8 bg-[#111315] p-6 transition-all hover:border-red-500/30 hover:shadow-xl hover:shadow-red-600/10">
              <div className="mb-4 grid h-11 w-11 place-items-center rounded-lg bg-red-600/10 border border-red-600/20 text-red-400 transition-transform group-hover:scale-110">
                <Icon size={20} />
              </div>
      ...
      <section className="py-16 border-b border-white/8">
        ...
        <h2 className="mt-3 text-3xl font-black tracking-tight text-white mb-12">How we got here</h2>
        <div className="relative">
          <div className="absolute left-[27px] top-0 bottom-0 w-px bg-white/8" />
            ...
            {timeline.map(({ year, title, desc }) => (
              <div key={year + title} className="flex gap-6">
                <div className="relative flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-xl border border-red-600/20 bg-red-600/10 text-center">
                  <span className="text-[10px] font-black uppercase tracking-widest text-red-400">{year}</span>
                </div>
                <div className="pt-1 pb-2">
                  <h3 className="font-black text-white">{title}</h3>
                  <p className="mt-1 text-sm text-white/50 leading-relaxed max-w-xl">{desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Subpage nav */}
      <section className="py-16 border-b border-white/8">
        <h2 className="text-2xl font-black text-white mb-8">Learn more about HubWeld</h2>
        <div className="grid gap-4 sm:grid-cols-3">
          {subpages.map(({ href, icon: Icon, title, desc }) => (
            <Link key={href} href={href} className="group flex flex-col gap-4 rounded-xl border border-white/8 bg-[#111315] p-6 transition-all hover:border-red-500/30 hover:shadow-xl hover:shadow-red-600/10">
              <div className="grid h-10 w-10 place-items-center rounded-lg border border-red-600/20 bg-red-600/10 text-red-400">
                <Icon size={18} />
              </div>
              <div>
                <div className="font-black text-white group-hover:text-red-400 transition-colors">{title}</div>
                <p className="mt-1 text-sm text-white/50 leading-relaxed">{desc}</p>
              </div>
              <div className="mt-auto flex items-center gap-1 text-xs font-bold text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <section className="py-16">
        <div className="rounded-xl border border-red-500/20 bg-red-600/5 p-10 text-center">
          <h2 className="text-3xl font-black text-white mb-3">Ready to join HubWeld?</h2>
          <p className="text-white/55 mb-8 max-w-lg mx-auto">Whether you're a welder, a supplier, or a company with projects to complete — there's a place for you on HubWeld.</p>
