    ...
    <div>
      {searchParams.success && (
        <div className="card mb-6 border-amber/40 bg-amber/10">
          ...
          <p className="text-sm text-white/80">Confirmation has been sent. Suppliers will be notified to fulfill your items.</p>
        </div>
      )}

      <Link href="/orders" className="text-sm text-white/50 hover:text-white">← All orders</Link>
      <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
        ...
        <span className="badge bg-white/10">{order.status}</span>
      </div>
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-[1fr_320px]">
          ...
          {order.items.map((it) => (
            <li key={it.id} className="card flex items-center justify-between gap-4">
              <div>
                ...
                <div className="text-xs text-white/60">From {it.supplier.supplierProfile?.businessName || it.supplier.name} · Qty {it.quantity}</div>
                <div className="text-xs text-white/40">{it.status}</div>
              </div>
        ...
        <aside className="card h-fit space-y-2 text-sm">
          ...
          <div className="mt-4 text-xs text-white/60">
            <div className="mb-1 font-bold text-white/80">Shipping to</div>
...
function Row({ label, value, bold }: { label: string; value: string; bold?: boolean }) {
  return (
    <div className={`flex justify-between ${bold ? "font-black text-base" : "text-white/70"}`}>
