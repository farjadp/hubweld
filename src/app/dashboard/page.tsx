  ...
  if (me.role === "CUSTOMER") {
    ...
    return (
      <Wrapper title={`Welcome, ${me.name}`} subtitle="Your posted jobs and bids overview.">
        ...
        {jobs.length === 0 ? <p className="text-slate-600">No jobs yet. Post your first one.</p> : (
          <ul className="grid gap-3">{jobs.map((j) => (
            <li key={j.id} className="card flex items-center justify-between">
              <div><Link href={`/jobs/${j.id}`} className="font-bold hover:text-amber">{j.title}</Link><div className="text-xs text-slate-600">{j.city} · {j.category} · {j._count.bids} bids</div></div>
              <span className="badge bg-slate-100">{j.status}</span>
            </li>
          ))}</ul>
        )}
      </Wrapper>
  ...
  if (me.role === "WELDER") {
    ...
    return (
      <Wrapper title={`Welcome, ${me.name}`} subtitle="Your active jobs, bids, and profile.">
        ...
        {bids.length === 0 ? <p className="text-slate-600">No bids yet. Browse open jobs.</p> : (
          <ul className="grid gap-3">
...
function Stats({ items }: { items: [string, number][] }) {
  return (
    <div className="grid gap-3 md:grid-cols-3">
      {items.map(([l, v]) => <div key={l} className="card"><div className="text-3xl font-black">{v}</div><div className="text-sm text-slate-600">{l}</div></div>)}
    </div>
  );
}
