...
export default function ProductForm({
  mode, productId, categories, initial,
}: {
  mode: "create" | "edit";
  productId?: string;
  ...
  const [form, setForm] = useState<Initial>(initial ?? { ...blank, categoryId: categories[0]?.id ?? "" });
  const [priceDollars, setPriceDollars] = useState((form.priceCents / 100).toFixed(2));
  const [err, setErr] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);

  function set<K extends keyof Initial>(k: K, v: Initial[K]) { setForm((f) => ({ ...f, [k]: v })); }
  ...
  return (
    <form onSubmit={submit} className="card space-y-4">
      <div className="grid gap-4 md:grid-cols-2">
        <div className="md:col-span-2"><label className="label">Product name</label>
        ...
        <div><label className="label">URL slug</label><input className="input" required value={form.slug} onChange={(e) => set("slug", e.target.value)} placeholder="arcstream-250-mig" /></div>
        ...
        <div className="flex items-end"><label className="flex items-center gap-2 text-sm">
