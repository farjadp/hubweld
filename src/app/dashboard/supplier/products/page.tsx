    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-white/60">{products.length} listings</p>
        </div>
        <Link href="/dashboard/supplier/products/new" className="btn-primary">+ New product</Link>
      </div>
      {products.length === 0 ? (
        <div className="card text-center">
          <p className="text-white/70">You haven't listed any products yet.</p>
        ...
        <ul className="grid gap-2">
          {products.map((p) => (
            <li key={p.id} className="card flex items-center gap-4">
              <div className="h-14 w-14 shrink-0 overflow-hidden rounded-lg bg-white/5">{p.imageUrl && <img src={p.imageUrl} alt="" className="h-full w-full object-cover" />}</div>
              <div className="flex-1 min-w-0">
                <Link href={`/shop/p/${p.slug}`} className="font-bold hover:text-amber line-clamp-1">{p.name}</Link>
                <div className="text-xs text-white/60">{p.category.name} · SKU {p.sku} · Stock {p.stock}</div>
              </div>
              <div className="text-right">
                <div className="font-black">{formatCents(p.priceCents)}</div>
                <div className="text-xs"><span className={`badge ${p.status === "ACTIVE" ? "bg-amber/20 text-amber" : "bg-white/10"}`}>{p.status}</span></div>
              </div>
