...
export default async function SupplierOrdersPage() {
  ...
  return (
    <div>
      ...
      <p className="mb-6 text-white/60">{items.length} line items to fulfill</p>
      {items.length === 0 ? (
          ...
          {items.map((it) => (
            <li key={it.id} className="card flex flex-wrap items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="text-xs text-white/60">Order #{it.orderId.slice(-8).toUpperCase()} · Buyer: {it.order.buyer.name} · {new Date(it.order.createdAt).toLocaleDateString()}</div>
                <div className="text-xs text-white/50">Ship to: {it.order.shipCity}, {it.order.shipRegion} {it.order.shipCountry}</div>
              </div>
              <div className="text-right">
                <div className="font-black">{formatCents(it.priceCents * it.quantity)}</div>
                <div className="text-xs text-white/50">Qty {it.quantity}</div>
                <span className="badge mt-1 bg-white/10 text-xs">{it.status}</span>
              </div>
            </li>
          ))}
        </ul>
